...
function ModalFactura() {
  ...
  const [selectedCityAIR2, setSelectedCityAIR2] = useState('');
...
function handleChange(event) {
  // Assuming you have a way to set the selected city in your component's state
  setSelectedCityAIR2(event.target.value);
}
  return (
    <>
          ...
          {serviceType === "Vuelo" && (
                  ...
                  {airports.length > 0 && (
                      <Form.Group required controlId="editForm.BuscaHotel">
                        <div className="row">
                        <Form.Label>Buscar Aeropuerto por ciudad</Form.Label>
                        </div>
                      <Form.Select required value={selectedCityAIR2} onChange={handleChange} name="Nombreaereopuerto">
                      {airports.map((index) => (
